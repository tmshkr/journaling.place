...
if [[ "$before" == "null" || "$after" == "null" ]]; then
    if images=$(aws ecr describe-images --repository-name "${GITHUB_REPOSITORY#*/}" --image-ids imageTag=$GITHUB_REF_NAME 2>&1); then
        tags=$(echo $images | jq -r '.imageDetails[0].imageTags[]')

        for tag in $tags; do
            if [[ $tag == "$GITHUB_REF_NAME-"* ]]; then
                before="${tag#"$GITHUB_REF_NAME-"}"
                after=$GITHUB_SHA
                break
            fi
        done
        ...
        if [[ "$images" == *"ImageNotFoundException"* ]]; then
            echo "No images found with tag $GITHUB_REF_NAME."
            exit 0
        else
