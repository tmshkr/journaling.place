...
prismaMock.user.create.mockImplementation((args): any => {
  ...
  const user: User = {
    ...
    salt: null,
    email: data!.email,
    emailNotifications: ["prompt_of_the_day"],
    createdAt: new Date(),
    updatedAt: new Date(),
  };
...
prismaMock.journal.update.mockImplementation((args): any => {
  ...
  const updatedJournal: Journal = {
    ...
    ciphertext: (ciphertext as unknown as Uint8Array<ArrayBuffer>) || journal.ciphertext,
    iv: (iv as unknown as Uint8Array<ArrayBuffer>) || journal.iv,
